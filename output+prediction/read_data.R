...
### READ
train <- fread('train.csv', select=c('TRIP_ID', 'TIMESTAMP', 'MISSING_DATA', 'POLYLINE'), stringsAsFactors=F,nrows=100000)
test <- fread('test.csv', select=c('TRIP_ID', 'TIMESTAMP', 'MISSING_DATA', 'POLYLINE'), stringsAsFactors=F)
train[, id:=-seq(.N, 1, -1)]
test[, id:=1:.N]
